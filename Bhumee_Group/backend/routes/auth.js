...
router.post("/createuser", async (req, res) => {
  try {
    ...
    const sqlCreateUser = `
    INSERT INTO users_master (name, phone, email, password,role,subrole,ORG) 
    VALUES (?, ?, ?, ?,?,?,?)
  `;
    ...
    // console.log("hashedPassword  ==> ", hashedPassword);
    const [userResult] = await db.query(sqlCreateUser, [name, phone, email, hashedPassword, role, subrole,password]);
    // console.log("Inserted user ID ==> ", userResult.insertId);
    const userid = userResult.insertId;
    // console.log("userid ==> ", userid);
    // console.log("userResult ==> ", userResult);
...
router.get('/get-all', async (req, res) => {
  ...
  const sqlQuery = 'SELECT * FROM users_master ORDER BY userid DESC';
...
router.put('/change-password', async (req, res) => {
  ...
  const isoldPasswordCorrect = await bcrypt.compare(oldPassword, rows[0].password);

  if (!isoldPasswordCorrect) {

    return res.status(201).json({ message: "Invalid id and  old password" });
  ...
  const sqlUpdateUser = 'UPDATE users_master SET password = ?, ORG = ? WHERE userid = ?';

  if (isPasswordCorrect) {
    const hashedPassword = await bcrypt.hash(newPassword, 10);
    const [rows] = await db.query(sqlUpdateUser, [hashedPassword, newPassword, id]);
    // console.log("rows", rows)
